...
int main(void)
{
    ...
    initSPI();		//Initialize SPI after clock
    spiWrite(0x05, 0x00);
    readdata = spiRead(0x00);
    _EINT(); 
    LPM3;
}
...
void initSPI(void)
{
    P3SEL |= BIT0 | BIT1 | BIT2 | BIT3;	//UCB0SIMO, UCB0SOMI, UCB0CLK, UCB0STE
    P3DIR |= BIT0 | BIT1 | BIT3;	//STE, MOSI, and CLK pins as output
...
void spiWrite(unsigned char data, unsigned char address)
{
    ...
    P1OUT &= ~BIT3;
    UCB0TXBUF =  (0b01000000 | address);//write command form TDC datasheet - initiates write to address (TDCCONFIG1 at 0x00)
...
int spiRead(unsigned char address)
{
    ...
    while (!(UCB0IFG & UCTXIFG));	//Wait until TX buffer is empty
    P1OUT &= ~BIT3;
    UCB0TXBUF =  (0x00 | address);//read command
    while (!(UCB0IFG & UCTXIFG));
    UCB0TXBUF = 0; // writes dummy data 
    while (!(UCB0IFG & UCTXIFG));
